...
class QRCodeController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($active_plan != null) {
            // Get User QR Codes
            $qr_codes = QrCode::where('user_id', Auth::user()->id)->where('type', '!=', 'epc')->where('status', '!=', 2)->orderBy('id', 'desc')->get();
    ...
    public function CreateQr()
    {
        ...
        if ($validity == 1) {
            ...
            // Redirect
            return redirect()->route('user.all.qr')->with('failed', trans('Your plan is over. Choose your plan renewal or new package and use it.'));
        }
    }
    ...
    public function saveQr(Request $request)
    {
        ...
        if ($request->qrcode_type == 'sms') {
            ...
            // Generate QR
            $generateQrCode = "sms:" . $phone . "?body=" . $content;
    ...
    public function editQr($id)
    {
        ...
        if ($qr_code_details) {
            ...
            return redirect()->route('user.all.qr')->with('failed', trans('No data available'));
        }
    }
    ...
    public function updateQr(Request $request)
    {
        ...
        // Redirect url
        return redirect()->back()->with('success', trans('QR Code updated successfully.'));
    }
            ...
            if ($validity == 1) {
                if ($qr_details == null) {
                    ...
                    if ($qr_code_count < $no_qrcodes) {
                        ...
                        return redirect()->back()->with('success', trans('QR Code Status Updated Successfully!'));
                    } else {
                        // Update status
                        ...
                        return redirect()->back()->with('failed', trans('Maximum qrcode creation limit is exceeded, Please upgrade your plan.'));
                    }
                }
            } else {
                // Redirect
    ...
    {
        // Update status
        QrCode::where('qr_code_id', $request->query('id'))->where('user_id', Auth::user()->id)->update([
            'status' => 2
        ]);
        QrCodeItem::where('qr_code_id', $request->query('id'))->where('user_id', Auth::user()->id)->update([
            'status' => 2
        ]);

        return redirect()->back()->with('success', trans('QR Code Deleted Successfully!'));
    }
