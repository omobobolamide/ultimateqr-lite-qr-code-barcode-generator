...
class BulkUploadController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($validity == 1) {
            if ($bar_code_counts < $no_barcodes) {
                ...
                // View page
                return view('user.pages.bulk-upload.index', compact('groups', 'settings'));
            } else {
    ...
    public function importBulkUpload(Request $request)
    {
        ...
        for ($i = 1; $i < count($barcodes); $i++) {
            ...
            // Required name
            if (!$name) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode name is required on the row number ' . $i));
            }

            // Required barcode type
            if (!$type) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode type is required on the row number ' . $i));
            }

            // Required barcode format
            if (!$format) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode format is required on the row number ' . $i));
            }

            // Required value
            if (!$value) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode value is required on the row number ' . $i));
            }

            // Required width
            if (!$width) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode width is required on the row number ' . $i));
            }

            // Required height
            if (!$height) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode height is required on the row number ' . $i));
            }

            // Required show text
            if (!$showText) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode show text is required on the row number ' . $i));
            }

            // Check max length of "Name"
            if (strlen($name) > 191) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode name minimum length of 1 or max length of 191 on the row number ' . $i));
            }

            // Check barcode types
            if ($type != "DNS1D" && $type != "DNS2D") {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Invalid barcode type on the row number ' . $i));
            }
            ...
            if (!in_array($format, $availableFormats)) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Invalid barcode format on the row number ' . $i));
            }

            // Check max length of "Value"
            if (strlen($value) > 191) {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Barcode value minimum length of 1 or max length of 191 on the row number ' . $i));
            }

            // Check show text
            if ($showText != "yes" && $showText != "no") {
                return redirect()->route('user.bulk.upload')->with('failed', trans('Invalid show text value on the row number ' . $i));
            }
        ...
        return redirect()->route('user.bulk.upload')->with('success', trans('Barcode generated successfully.'));
    }
}
