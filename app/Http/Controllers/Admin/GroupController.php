...
class GroupController extends Controller
{
    ...
    public function index()
    {
        ...
        // View page
        return view('admin.pages.groups.index', compact('groups', 'settings'));
    }
    ...
    public function saveGroup(Request $request)
    {
        ...
        return redirect()->route('admin.create.group')->with('success', trans('Group created successfully.'));
    }
    ...
    public function viewGroup(Request $request, $id)
    {
        ...
        if ($groupBarcodes) {
            // View media
            return view('admin.pages.groups.view', compact('groupBarcodes', 'settings'));
        } else {
            // Redirect
            return redirect()->route('admin.all.groups')->with('failed', trans('No data available'));
        }
    }
    ...
    public function editGroup(Request $request, $id)
    {
        ...
        // Check data 
        if ($group_details) {
            return view('admin.pages.groups.edit', compact('group_details'));
        } else {
            // Redirect
            return redirect()->route('admin.all.groups')->with('failed', trans('No data available'));
        }
    ...
    public function updateGroup(Request $request)
    {
        ...
        return redirect()->route('admin.edit.group', $group_id)->with('success', trans('Group updated successfully.'));
    }
    ...
    public function downloadGroup(Request $request, $id)
    {
        ...
        if (count($groupBarcodes) > 0) {
            // Delete folder
            File::deleteDirectory(public_path('/images/admin/barcode/' . $groupBarcodes[0]->group_id));
            ...
            for ($i = 0; $i < count($groupBarcodes); $i++) {
                ...
                $path = public_path() . '/images/admin/barcode/' . $groupId;
                File::makeDirectory($path, 0777, true, true);

                // Convert to png image
                $image = str_replace(' ', '+', $image);
                $imageName = $barcodeId . '.' . 'png';
                File::put(public_path('images/admin/barcode/' . $groupId) . '/' . $imageName, base64_decode($result));
            }
            ...
            if ($zip->open(public_path($zipName), \ZipArchive::CREATE) == TRUE) {
                $files = File::files(public_path('images/admin/barcode/' . $id));
