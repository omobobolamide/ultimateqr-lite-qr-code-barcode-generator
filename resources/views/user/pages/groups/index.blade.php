            ...
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto ms-auto d-print-none">
                        <div class="d-flex">
                            <a href="{{ route('user.create.group') }}" class="btn btn-primary">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($groups as $group)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('user.view.group', $group->group_id) }}">{{ $group->group_name }}</a></td>
                                                </td>
                                                @if ($group->status == 1)
                                                    <td><span
                                                    ...
                                                    <span class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                                            <a class="dropdown-item"
                                                                href="{{ route('user.edit.group', $group->group_id) }}">{{ __('Edit') }}</a>
                                                            <a class="dropdown-item"
                                                                href="{{ route('user.view.group', $group->group_id) }}">{{ __('View') }}</a>
                                                            <a class="dropdown-item"
                                                                href="{{ route('user.download.group', $group->group_id) }}">{{ __('Download Barcodes') }}</a>
                                                            @if ($group->status == 0)
...
@section('custom-js')
    <script>
        ...
        function updateGroupStatus(statusGroupId, statusGroupStatus) {
            ...
            status_link.setAttribute("href", "{{ route('user.update.group.status') }}?id=" + statusGroupId);
        }
        ...
        function deleteGroup(deleteGroupId, deleteGroupStatus) {
            ...
            delete_link.setAttribute("href", "{{ route('user.delete.group') }}?id=" + deleteGroupId);
        }
    </script>
@endsection
@endsection
