@extends('user.layouts.app')

{{-- Custom CSS --}}
@section('custom-css')
    <style>
        #loader {
            display: flex;
            text-align: center;
            justify-content: center;
        }
    </style>
@endsection

@section('content')
    <div class="page-wrapper">
        <div class="container-xl">
            <!-- Page title -->
            <div class="page-header d-print-none">
                <div class="row align-items-center">
                    <div class="col">
                        <div class="page-pretitle">
                            {{ __('Overview') }}
                        </div>
                        <h2 class="page-title">
                            {{ __('Create QR Code') }}
                        </h2>
                    </div>
                </div>
            </div>
        </div>
        <div class="page-body">
            <div class="container-xl">

                {{-- Failed --}}
                @if (Session::has('failed'))
                    <div class="alert alert-important alert-danger alert-dismissible" role="alert">
                        <div class="d-flex">
                            <div>
                                {{ Session::get('failed') }}
                            </div>
                        </div>
                        <a class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="close"></a>
                    </div>
                @endif

                {{-- Success --}}
                @if (Session::has('success'))
                    <div class="alert alert-important alert-success alert-dismissible" role="alert">
                        <div class="d-flex">
                            <div>
                                {{ Session::get('success') }}
                            </div>
                        </div>
                        <a class="btn-close btn-close-white" data-bs-dismiss="alert" aria-label="close"></a>
                    </div>
                @endif

                <div class="row row-deck row-cards">
                    <div class="col-sm-12 col-lg-12">
                        <div class="col">
                            {{-- Tabs Content --}}
                            <div class="col">
                                {{-- Text --}}
                                <form action="{{ route('user.save.qr') }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <div class="col">
                                        <div class="row">

                                            {{-- Show Generated QR Code --}}
                                            <div class="col-xl-6 col-lg-6 col-md-6">
                                                <div class="sticky-top card p-3">
                                                    {{-- Regenerate QR Code --}}
                                                    <div id="regenerate_qr" class="visible-print p-3 text-center">
                                                        <img src="data:image/png;base64, {!! base64_encode(
                                                            QrCode::format('png')->size('400')->merge('images/web/social/google_logo_new.png', 0.2, true)->generate('https://google.co.in'),
                                                        ) !!} ">
                                                    </div>

                                                    {{-- Loader --}}
                                                    <div id="loader">
                                                        <img
                                                            src="data:image/gif;base64,">
                                                    </div>
                                                    <br>


                                                    {{-- Download Formats --}}
                                                    <div class="row g-2 align-items-center mb-3 mt-3">
                                                        <div class="col-12 col-xl-2 mb-3 font-weight-semibold">
                                                            {{ __('Download Formats') }}</div>

                                                        {{-- .PNG --}}
                                                        <div class="col-6 col-sm-4 col-md-2 col-xl mb-3">
                                                            <a href="#" onclick="downloadQR('png')"
                                                                class="btn btn-pink w-100">
                                                                {{ __('.PNG') }}
                                                            </a>
                                                        </div>

                                                        {{-- .JPG --}}
                                                        <div class="col-6 col-sm-4 col-md-2 col-xl mb-3">
                                                            <a href="#" onclick="downloadQR('jpg')"
                                                                class="btn btn-secondary w-100">
                                                                {{ __('.JPG') }}
                                                            </a>
                                                        </div>

                                                        {{-- .SVG --}}
                                                        <div class="col-6 col-sm-4 col-md-2 col-xl mb-3">
                                                            <a href="#" onclick="downloadQR('svg')"
                                                                class="btn btn-success w-100">
                                                                {{ __('.SVG') }}
                                                            </a>
                                                        </div>

                                                        {{-- .WebP --}}
                                                        <div class="col-6 col-sm-4 col-md-2 col-xl mb-3">
                                                            <a href="#" onclick="downloadQR('webp')"
                                                                class="btn btn-warning w-100">
                                                                {{ __('.WebP') }}
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>


                                            {{-- Generate Form --}}
                                            <div class="col-xl-6 col-lg-6 col-md-6">
                                                <div class="card p-3">
                                                    <div class="row">

                                                        {{-- QR Code Type --}}
                                                        <div class="col-md-6 col-xl-6">
                                                            <div class="mb-3">
                                                                <label
                                                                    class="form-label required">{{ __('QR Code Type') }}</label>
                                                                <select name="qrcode_type"
                                                                    onchange="redirectQrType(this.value)"
                                                                    class="form-select" required>
                                                                    {{-- Check access to "TEXT" --}}
                                                                    @if (json_decode(Auth::user()->plan_details)->access_text == 1)
                                                                        <option value="text">{{ __('Plain Text') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_phone == 1)
                                                                        <option value="phone">{{ __('Phone Number') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_email == 1)
                                                                        <option value="email">{{ __('Email') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_whatsapp == 1)
                                                                        <option value="whatsapp">
                                                                            {{ __('WhatsApp Message') }}</option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_url == 1)
                                                                        <option value="url">{{ __('URL') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_sms == 1)
                                                                        <option value="sms">{{ __('Text Message') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_location == 1)
                                                                        <option value="location">{{ __('Map Location') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_facetime == 1)
                                                                        <option value="facetime">{{ __('Facetime') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_wifi == 1)
                                                                        <option value="wifi">{{ __('WiFi') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_event == 1)
                                                                        <option value="event">{{ __('Event') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_vcard == 1)
                                                                        <option value="vcard">{{ __('Save Contact') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_crypto == 1)
                                                                        <option value="crypto">{{ __('Crypto Payment') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_upi == 1)
                                                                        <option value="upi">{{ __('UPI Payment') }}
                                                                        </option>
                                                                    @endif

                                                                    @if (json_decode(Auth::user()->plan_details)->access_paypal == 1)
                                                                        <option value="paypal">{{ __('Paypal Payment') }}
                                                                        </option>
                                                                    @endif
                                                                </select>
                                                            </div>
                                                        </div>

                                                        {{-- Selected Type --}}
                                                        <input type="hidden" class="form-control"
                                                            id="selectedQrCodeType">

                                                        {{-- Name --}}
                                                        <div class="col-md-6 col-xl-6">
                                                            <div class="mb-3">
                                                                <label
                                                                    class="form-label required">{{ __('Name') }}</label>
                                                                <input type="text" class="form-control" name="name"
                                                                    maxlength="100" value="{{ uniqid() }}"
                                                                    id="name" placeholder="{{ __('Name') }}"
                                                                    required>
                                                            </div>
                                                        </div>

                                                        {{-- Code Generate --}}
                                                        <div id="generate"></div>

                                                        {{-- Advance Settings --}}
                                                        <div class="col">
                                                            <div class="card">
                                                                <div class="card-body">
                                                                    <div class="card-title">{{ __('Advanced Settings') }}
                                                                    </div>

                                                                    {{-- QR code settings --}}
                                                                    <div class="pt-1">
                                                                        {{-- QR Code Style --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('QR Code Style') }}</label>
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_style"
                                                                                            id="qrcode_style"
                                                                                            value="square"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label"><!-- Download SVG icon from http://tabler-icons.io/i/home -->

                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-square"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/square
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <rect x="4" y="4"
                                                                                                    width="16"
                                                                                                    height="16"
                                                                                                    rx="2"></rect>
                                                                                            </svg>

                                                                                            {{ __('Square') }}</span>
                                                                                    </label>

                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_style"
                                                                                            id="qrcode_style"
                                                                                            value="dot"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label"><!-- Download SVG icon from http://tabler-icons.io/i/home -->
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon me-1 icon-tabler icon-tabler-grain"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/grain
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <circle cx="4.5"
                                                                                                    cy="9.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="9.5"
                                                                                                    cy="4.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="9.5"
                                                                                                    cy="14.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="4.5"
                                                                                                    cy="19.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="14.5"
                                                                                                    cy="9.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="19.5"
                                                                                                    cy="4.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="14.5"
                                                                                                    cy="19.5" r="1">
                                                                                                </circle>
                                                                                                <circle cx="19.5"
                                                                                                    cy="14.5" r="1">
                                                                                                </circle>
                                                                                            </svg>
                                                                                            {{ __('Dot') }}</span>
                                                                                    </label>

                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_style"
                                                                                            id="qrcode_style"
                                                                                            value="round"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label"><!-- Download SVG icon from http://tabler-icons.io/i/home -->
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-border-radius"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/border-radius
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <path
                                                                                                    d="M4 12v-4a4 4 0 0 1 4 -4h4">
                                                                                                </path>
                                                                                                <line x1="16"
                                                                                                    y1="4"
                                                                                                    x2="16"
                                                                                                    y2="4.01"></line>
                                                                                                <line x1="20"
                                                                                                    y1="4"
                                                                                                    x2="20"
                                                                                                    y2="4.01"></line>
                                                                                                <line x1="20"
                                                                                                    y1="8"
                                                                                                    x2="20"
                                                                                                    y2="8.01"></line>
                                                                                                <line x1="20"
                                                                                                    y1="12"
                                                                                                    x2="20"
                                                                                                    y2="12.01"></line>
                                                                                                <line x1="4"
                                                                                                    y1="16"
                                                                                                    x2="4"
                                                                                                    y2="16.01"></line>
                                                                                                <line x1="20"
                                                                                                    y1="16"
                                                                                                    x2="20"
                                                                                                    y2="16.01"></line>
                                                                                                <line x1="4"
                                                                                                    y1="20"
                                                                                                    x2="4"
                                                                                                    y2="20.01"></line>
                                                                                                <line x1="8"
                                                                                                    y1="20"
                                                                                                    x2="8"
                                                                                                    y2="20.01"></line>
                                                                                                <line x1="12"
                                                                                                    y1="20"
                                                                                                    x2="12"
                                                                                                    y2="20.01"></line>
                                                                                                <line x1="16"
                                                                                                    y1="20"
                                                                                                    x2="16"
                                                                                                    y2="20.01"></line>
                                                                                                <line x1="20"
                                                                                                    y1="20"
                                                                                                    x2="20"
                                                                                                    y2="20.01"></line>
                                                                                            </svg>
                                                                                            {{ __('Round') }}</span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Color Style --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Color Style') }}</label>
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="color_style"
                                                                                            value="color"
                                                                                            onclick="checkColor(this); regenerateQrCode();"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label"><!-- Download SVG icon from http://tabler-icons.io/i/home -->

                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-brush"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/brush
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <path
                                                                                                    d="M3 21v-4a4 4 0 1 1 4 4h-4">
                                                                                                </path>
                                                                                                <path
                                                                                                    d="M21 3a16 16 0 0 0 -12.8 10.2">
                                                                                                </path>
                                                                                                <path
                                                                                                    d="M21 3a16 16 0 0 1 -10.2 12.8">
                                                                                                </path>
                                                                                                <path
                                                                                                    d="M10.6 9a9 9 0 0 1 4.4 4.4">
                                                                                                </path>
                                                                                            </svg>

                                                                                            {{ __('Single Color') }}</span>
                                                                                    </label>

                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="color_style"
                                                                                            value="gradient"
                                                                                            onclick="checkColor(this); regenerateQrCode();"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label"><!-- Download SVG icon from http://tabler-icons.io/i/home -->

                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-palette"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/palette
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <path
                                                                                                    d="M12 21a9 9 0 1 1 0 -18a9 8 0 0 1 9 8a4.5 4 0 0 1 -4.5 4h-2.5a2 2 0 0 0 -1 3.75a1.3 1.3 0 0 1 -1 2.25">
                                                                                                </path>
                                                                                                <circle cx="7.5"
                                                                                                    cy="10.5" r=".5"
                                                                                                    fill="currentColor">
                                                                                                </circle>
                                                                                                <circle cx="12"
                                                                                                    cy="7.5" r=".5"
                                                                                                    fill="currentColor">
                                                                                                </circle>
                                                                                                <circle cx="16.5"
                                                                                                    cy="10.5" r=".5"
                                                                                                    fill="currentColor">
                                                                                                </circle>
                                                                                            </svg>
                                                                                            {{ __('Gradient Color') }}</span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Color --}}
                                                                        <div class="col-md-12 col-xl-12 color-form-group">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Color') }}</label>
                                                                                <input type="color" class="form-control"
                                                                                    name="color" id="color"
                                                                                    value="#000000"
                                                                                    onchange="regenerateQrCode()"
                                                                                    placeholder="{{ __('Foreground color') }}">
                                                                            </div>
                                                                        </div>

                                                                        {{-- Gradient type --}}
                                                                        <div
                                                                            class="col-md-12 col-xl-12 gradient-form-group">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Gradient type') }}</label>
                                                                                {{-- Vertical --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="foreground_gradient_type"
                                                                                            id="foreground_gradient_type"
                                                                                            value="vertical"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Vertical') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Vertical --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="foreground_gradient_type"
                                                                                            id="foreground_gradient_type"
                                                                                            value="horizontal"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Horizontal') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Diagonal --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="foreground_gradient_type"
                                                                                            id="foreground_gradient_type"
                                                                                            value="diagonal"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Diagonal') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Inverse diagonal --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="foreground_gradient_type"
                                                                                            id="foreground_gradient_type"
                                                                                            value="inverse_diagonal"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Inverse diagonal') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Radial --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="foreground_gradient_type"
                                                                                            id="foreground_gradient_type"
                                                                                            value="radial"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Radial') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Primary color --}}
                                                                        <div
                                                                            class="col-md-12 col-xl-12 gradient-form-group">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Primary color') }}</label>
                                                                                <input type="color" class="form-control"
                                                                                    name="primary_color"
                                                                                    id="primary_color" value="#0fcc8d"
                                                                                    onchange="regenerateQrCode()"
                                                                                    placeholder="{{ __('Primary color') }}">
                                                                            </div>
                                                                        </div>

                                                                        {{-- Secondary color --}}
                                                                        <div
                                                                            class="col-md-12 col-xl-12 gradient-form-group">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Secondary color') }}</label>
                                                                                <input type="color" class="form-control"
                                                                                    name="secondary_color"
                                                                                    id="secondary_color" value="#088040"
                                                                                    onchange="regenerateQrCode()"
                                                                                    placeholder="{{ __('Secondary color') }}">
                                                                            </div>
                                                                        </div>

                                                                        {{-- Need eye color? --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Need eye color?') }}</label>
                                                                                {{-- Yes --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="need_eye_color"
                                                                                            id="need_eye_color"
                                                                                            value="1"
                                                                                            onchange="checkEyeColor(this.value); regenerateQrCode();"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Yes') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- No --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="need_eye_color"
                                                                                            id="need_eye_color"
                                                                                            value="0"
                                                                                            onchange="checkEyeColor(this.value); regenerateQrCode();"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('No') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Change the eye colors --}}
                                                                        <div class="col-md-12 col-xl-12 eyes_control">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Change the eye colors') }}</label>
                                                                                <input type="color" class="form-control"
                                                                                    name="eye_color" id="eye_color"
                                                                                    value="#000000"
                                                                                    onchange="regenerateQrCode()"
                                                                                    placeholder="{{ __('Change the eye colors') }}">
                                                                            </div>
                                                                        </div>

                                                                        {{-- EyeColor Position --}}
                                                                        <div class="col-md-12 col-xl-12 eyes_control">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Eye Color Position') }}</label>
                                                                                {{-- Left --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="eyeColor_position"
                                                                                            id="eyeColor_position"
                                                                                            value="0"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-float-left"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/float-left
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <rect width="6"
                                                                                                    height="6" x="4"
                                                                                                    y="5" rx="1">
                                                                                                </rect>
                                                                                                <line x1="14"
                                                                                                    y1="7"
                                                                                                    x2="20"
                                                                                                    y2="7"></line>
                                                                                                <line x1="14"
                                                                                                    y1="11"
                                                                                                    x2="20"
                                                                                                    y2="11"></line>
                                                                                                <line x1="4"
                                                                                                    y1="15"
                                                                                                    x2="20"
                                                                                                    y2="15"></line>
                                                                                                <line x1="4"
                                                                                                    y1="19"
                                                                                                    x2="20"
                                                                                                    y2="19"></line>
                                                                                            </svg>
                                                                                            {{ __('Left') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Right --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="eyeColor_position"
                                                                                            id="eyeColor_position"
                                                                                            value="1"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-float-right"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/float-right
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <rect width="6"
                                                                                                    height="6" x="14"
                                                                                                    y="5" rx="1">
                                                                                                </rect>
                                                                                                <line x1="4"
                                                                                                    y1="7"
                                                                                                    x2="10"
                                                                                                    y2="7"></line>
                                                                                                <line x1="4"
                                                                                                    y1="11"
                                                                                                    x2="10"
                                                                                                    y2="11"></line>
                                                                                                <line x1="4"
                                                                                                    y1="15"
                                                                                                    x2="20"
                                                                                                    y2="15"></line>
                                                                                                <line x1="4"
                                                                                                    y1="19"
                                                                                                    x2="20"
                                                                                                    y2="19"></line>
                                                                                            </svg>
                                                                                            {{ __('Right') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Bottom --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="eyeColor_position"
                                                                                            id="eyeColor_position"
                                                                                            value="2"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-float-center"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/float-center
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <rect width="6"
                                                                                                    height="6" x="9"
                                                                                                    y="5" rx="1">
                                                                                                </rect>
                                                                                                <line x1="4"
                                                                                                    y1="7"
                                                                                                    x2="5"
                                                                                                    y2="7"></line>
                                                                                                <line x1="4"
                                                                                                    y1="11"
                                                                                                    x2="5"
                                                                                                    y2="11"></line>
                                                                                                <line x1="19"
                                                                                                    y1="7"
                                                                                                    x2="20"
                                                                                                    y2="7"></line>
                                                                                                <line x1="19"
                                                                                                    y1="11"
                                                                                                    x2="20"
                                                                                                    y2="11"></line>
                                                                                                <line x1="4"
                                                                                                    y1="15"
                                                                                                    x2="20"
                                                                                                    y2="15"></line>
                                                                                                <line x1="4"
                                                                                                    y1="19"
                                                                                                    x2="20"
                                                                                                    y2="19"></line>
                                                                                            </svg>
                                                                                            {{ __('Bottom') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Eye Color Style --}}
                                                                        <div class="col-md-12 col-xl-12 eyes_control">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Eye Color Style') }}</label>
                                                                                {{-- Square --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="eye_color_style"
                                                                                            id="eye_color_style"
                                                                                            value="square"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-square"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/square
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <rect x="4" y="4"
                                                                                                    width="16"
                                                                                                    height="16"
                                                                                                    rx="2"></rect>
                                                                                            </svg>
                                                                                            {{ __('Square') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Circle --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="eye_color_style"
                                                                                            id="eye_color_style"
                                                                                            value="circle"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                                                class="icon icon-tabler me-1 icon-tabler-circle"
                                                                                                width="24"
                                                                                                height="24"
                                                                                                viewBox="0 0 24 24"
                                                                                                stroke-width="2"
                                                                                                stroke="currentColor"
                                                                                                fill="none"
                                                                                                stroke-linecap="round"
                                                                                                stroke-linejoin="round">
                                                                                                <desc>Download more icon
                                                                                                    variants from
                                                                                                    https://tabler-icons.io/i/circle
                                                                                                </desc>
                                                                                                <path stroke="none"
                                                                                                    d="M0 0h24v24H0z"
                                                                                                    fill="none"></path>
                                                                                                <circle cx="12"
                                                                                                    cy="12" r="9">
                                                                                                </circle>
                                                                                            </svg>
                                                                                            {{ __('Circle') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>

                                                                    {{-- Merge Logo --}}
                                                                    <div class="pt-1">
                                                                        {{-- Merge Logo --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <div class="form-label">
                                                                                    {{ __('Upload Logo') }}
                                                                                </div>
                                                                                <input type="file" class="form-control"
                                                                                    name="upload_logo" id="upload_logo"
                                                                                    accept=".jpg, .jpeg, .png, .gif"
                                                                                    onchange="regenerateQrCode()">
                                                                            </div>
                                                                        </div>

                                                                        {{-- Upload Logo size --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Upload Logo Size') }}</label>
                                                                                {{-- Small --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="upload_logo_size"
                                                                                            id="upload_logo_size"
                                                                                            value="0.1"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Small') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                                {{-- Medium --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="upload_logo_size"
                                                                                            id="upload_logo_size"
                                                                                            value="0.2"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Medium') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                                {{-- Large --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="upload_logo_size"
                                                                                            id="upload_logo_size"
                                                                                            value="0.5"
                                                                                            onclick="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Large') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>

                                                                    {{-- More settings --}}
                                                                    <div class="pt-1">
                                                                        {{-- Download Quality --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Download Quality') }}</label>
                                                                                {{-- Basic --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="size" id="size"
                                                                                            value="200"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Basic') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Normal --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="size" id="size"
                                                                                            value="500"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Normal') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Fine --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="size" id="size"
                                                                                            value="1000"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Fine') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- High --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="size" id="size"
                                                                                            value="1500"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('High') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Very High --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="size" id="size"
                                                                                            value="2000"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Very High') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>

                                                                        {{-- Level of error correction --}}
                                                                        <div class="col-md-12 col-xl-12">
                                                                            <div class="mb-3">
                                                                                <label
                                                                                    class="form-label">{{ __('Level of error correction') }}</label>
                                                                                {{-- Low --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_ecc"
                                                                                            id="qrcode_ecc" value="L"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input"
                                                                                            checked>
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Low') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Medium --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_ecc"
                                                                                            id="qrcode_ecc" value="M"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Medium') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- Quartile --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_ecc"
                                                                                            id="qrcode_ecc" value="Q"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('Quartile') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>

                                                                                {{-- High --}}
                                                                                <div class="form-selectgroup">
                                                                                    <label class="form-selectgroup-item">
                                                                                        <input type="radio"
                                                                                            name="qrcode_ecc"
                                                                                            id="qrcode_ecc" value="H"
                                                                                            onchange="regenerateQrCode()"
                                                                                            class="form-selectgroup-input">
                                                                                        <span
                                                                                            class="form-selectgroup-label">
                                                                                            {{ __('High') }}
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        <span
                                                            class="text-muted mt-3">{{ __('Note: Double-check your QR Code once before using it.') }}</span>
                                                        {{-- Generate QR Code --}}
                                                        <div class="text-end mt-3">
                                                            <div class="text-end">
                                                                <button type="submit"
                                                                    class="btn btn-primary btn-md ms-auto">
                                                                    {{ __('Generate and Save QR') }}
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        {{-- Footer --}}
        @include('user.includes.footer')
    </div>

    {{-- Custom JS --}}
@section('custom-js')
    <script>
        // Check current color type
        window.onload = checkColor(`single`);
        // Check current eye color
        window.onload = checkEyeColor(`0`);

        // Check gradient
        hideGradientElements();

        // Hide loader
        $("#loader").hide();
        $("#regenerate_qr").show();

        // Default Download Image
        $("#download_qr").attr("href",
            "data:image/png;base64,  "
        );

        // Default QR code Generation
        // Text
        var generateCode = '<div class="col-md-12 col-xl-12">\
                                    <div class="mb-3">\
                                        <label class="form-label required">{{ __('Content') }}</label>\
                                        <textarea name="content" id="content" minlength="1" maxlength="350" placeholder="Lorem ipsum..." class="form-control" cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>\
                                    </div>\
                                </div>';
        $('#generate').html(generateCode);
        $('#selectedQrCodeType').val('text');

        // Check QR Code type
        function redirectQrType(type) {
            "use strict";

            // Check TEXT
            if (type == "text") {
                // Text
                $('#selectedQrCodeType').val('text');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Content') }}</label>\
                                                <textarea name="content" id="content" minlength="1" maxlength="350" placeholder="Lorem ipsum..." class="form-control" cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check URL
            if (type == "url") {
                // URL
                $('#selectedQrCodeType').val('url');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Link Url') }}</label>\
                                                <input type="url" class="form-control" name="url" onchange="regenerateQrCode()" id="content" placeholder="{{ __('Ex: http://domain.com') }}" required>\
                                            </div>\
                                        </div>\
                                        <div data-bs-toggle="tooltip" data-bs-placement="top">\
                                            <div class="col-md-12 col-xl-12">\
                                                <div class="mb-3">\
                                                    <label class="form-check">\
                                                        <input class="form-check-input" type="checkbox" name="enable_analytics" id="enable_analytics" onclick="regenerateQrCode()">\
                                                        <span class="form-check-label">{{ __('Enable Analytics') }}</span>\
                                                    </label>\
                                                </div>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Phone
            if (type == "phone") {
                // Phone
                $('#selectedQrCodeType').val('phone');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Mobile Number') }}</label>\
                                                <input type="text" class="form-control" name="content" maxlength="25" onchange="regenerateQrCode()" id="content" placeholder="{{ __('Mobile Number') }}" required>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check SMS
            if (type == "sms") {
                // SMS
                $('#selectedQrCodeType').val('sms');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Mobile Number') }}</label>\
                                                <input type="text" class="form-control" name="phone" maxlength="25" onchange="regenerateQrCode()" id="phone" placeholder="{{ __('Mobile Number') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Message') }}</label>\
                                                <textarea name="content" id="content" class="form-control" maxlength="256" cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Email
            if (type == "email") {
                // Email
                $('#selectedQrCodeType').val('email');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Email ID') }}</label>\
                                                <input type="text" class="form-control" name="email" maxlength="50" onchange="regenerateQrCode()" id="email" placeholder="{{ __('Email ID') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Subject') }}</label>\
                                                <input type="text" class="form-control" name="subject" maxlength="50" onchange="regenerateQrCode()" id="subject" placeholder="{{ __('Subject') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Body') }}</label>\
                                                <textarea name="content" id="content" class="form-control" maxlength="300" cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check WhatsApp
            if (type == "whatsapp") {
                // WhatsApp
                $('#selectedQrCodeType').val('whatsapp');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('WhatsApp Number') }}</label>\
                                                <input type="text" class="form-control" name="phone" maxlength="25" onchange="regenerateQrCode()" id="phone" placeholder="{{ __('WhatsApp Number') }}" required>\
                                                <small class="mt-2">{{ __('With country code (For ex: 919876543210)') }}</small>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Message') }}</label>\
                                                <textarea name="content" id="content" class="form-control" maxlength="350" cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Facetime
            if (type == "facetime") {
                // Facetime
                $('#selectedQrCodeType').val('facetime');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Mobile number/email ID') }}</label>\
                                                <input type="text" class="form-control" name="content" onchange="regenerateQrCode()" id="content" placeholder="{{ __('Mobile number/email ID') }}" required>\
                                                <small class="mt-3">{{ __('Only support on iOS & MacOS Devices') }}</small>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Location
            if (type == "location") {
                // Location
                $('#selectedQrCodeType').val('location');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Latitude') }}</label>\
                                                <input type="text" class="form-control" name="latitude" maxlength="50" onchange="regenerateQrCode()" id="latitude" placeholder="{{ __('Latitude') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Longitude') }}</label>\
                                                <input type="text" class="form-control" name="longitude" maxlength="50" onchange="regenerateQrCode()" id="longitude" placeholder="{{ __('Longitude') }}" required>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check WiFi
            if (type == "wifi") {
                // WiFi
                $('#selectedQrCodeType').val('wifi');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Network Name (SSID)') }}</label>\
                                                <input type="text" class="form-control" name="wifi_ssid" maxlength="100" onchange="regenerateQrCode()" id="wifi_ssid" placeholder="{{ __('Network Name (SSID)') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label">{{ __('Security') }}</label>\
                                                <select class="form-select" name="wifi_encryption" id="wifi_encryption" onchange="regenerateQrCode()">\
                                                    <option value="nopass">{{ __('None') }}</option>\
                                                    <option value="WEP">{{ __('WEP') }}</option>\
                                                    <option value="WPA">{{ __('WPA/WPA2-Personal') }}</option>\
                                                </select>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Password') }}</label>\
                                                <input type="text" class="form-control" name="wifi_password" maxlength="25" onchange="regenerateQrCode()" id="wifi_password" placeholder="{{ __('Password') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label">{{ __('Hidden?') }}</label>\
                                                <select class="form-select" name="wifi_is_hidden" id="wifi_is_hidden" onchange="regenerateQrCode()">\
                                                    <option value="1">{{ __('Yes') }}</option>\
                                                    <option value="0">{{ __('No') }}</option>\
                                                </select>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Event
            if (type == "event") {
                // Event
                $('#selectedQrCodeType').val('event');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Title') }}</label>\
                                                <input type="text" class="form-control" name="event" onchange="regenerateQrCode()" id="event" placeholder="{{ __('Title') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Message') }}</label>\
                                                <textarea name="event_note" id="event_note" class="form-control" cols="30" rows="2" maxlength="220" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Location') }}</label>\
                                                <input type="text" class="form-control" name="event_location" onchange="regenerateQrCode()" id="event_location" placeholder="{{ __('Location') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('From') }}</label>\
                                                <input type="datetime-local" class="form-control" name="event_start_datetime" onchange="regenerateQrCode()" id="event_start_datetime" placeholder="{{ __('From') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('To') }}</label>\
                                                <input type="datetime-local" class="form-control" name="event_end_datetime" onchange="regenerateQrCode()" id="event_end_datetime" placeholder="{{ __('To') }}" required>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Crypto
            if (type == "crypto") {
                // Crypto
                $('#selectedQrCodeType').val('crypto');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Cryptocurrency') }}</label>\
                                                <select class="form-select" name="crypto_coin" id="crypto_coin" onchange="regenerateQrCode()" required>\
                                                    <option value="bitcoin">{{ __('Bitcoin') }}</option>\
                                                    <option value="bitcoincash">{{ __('Bitcoin cash address') }} </option>\
                                                    <option value="ethereum">{{ __('Ether') }}</option>\
                                                    <option value="litecoin">{{ __('Litecoin') }}</option>\
                                                    <option value="dash">{{ __('Dash') }}</option>\
                                                </select>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Receiver') }}</label>\
                                                <input type="text" class="form-control" name="crypto_address" maxlength="250" onchange="regenerateQrCode()" id="crypto_address" placeholder="{{ __('Receiver') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Amount') }}</label>\
                                                <input type="text" class="form-control" name="crypto_amount" maxlength="15" onchange="regenerateQrCode()" id="crypto_amount" placeholder="{{ __('Amount') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Message') }}</label>\
                                                <textarea name="crypto_msg" id="crypto_msg" class="form-control" cols="30" rows="2" maxlength="150" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Vcard
            if (type == "vcard") {
                // Vcard
                $('#selectedQrCodeType').val('vcard');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('First name') }}</label>\
                                                <input type="text" class="form-control" name="vcard_first_name" maxlength="40" onchange="regenerateQrCode()" id="vcard_first_name" placeholder="{{ __('First name') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Last name') }}</label>\
                                                <input type="text" class="form-control" name="vcard_last_name" maxlength="40" onchange="regenerateQrCode()" id="vcard_last_name" placeholder="{{ __('Last name') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Mobile number') }}</label>\
                                                <input type="text" class="form-control" name="vcard_phone" maxlength="20" onchange="regenerateQrCode()" id="vcard_phone" placeholder="{{ __('Mobile number') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Email Address') }}</label>\
                                                <input type="email" class="form-control" name="vcard_email" maxlength="50" onchange="regenerateQrCode()" id="vcard_email" placeholder="{{ __('Email Address') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Website Link') }}</label>\
                                                <input type="url" class="form-control" name="vcard_url" maxlength="50" onchange="regenerateQrCode()" id="vcard_url" placeholder="{{ __('Website Link') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Company Name') }}</label>\
                                                <input type="text" class="form-control" name="vcard_company" maxlength="50" onchange="regenerateQrCode()" id="vcard_company" placeholder="{{ __('Company Name') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Job position') }}</label>\
                                                <input type="text" class="form-control" name="vcard_job_title" onchange="regenerateQrCode()" maxlength="50" id="vcard_job_title" placeholder="{{ __('Job position') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('DOB') }}</label>\
                                                <input type="date" class="form-control" name="vcard_birthday" onchange="regenerateQrCode()" id="vcard_birthday" placeholder="{{ __('DOB') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Street') }}</label>\
                                                <input type="text" class="form-control" name="vcard_street" onchange="regenerateQrCode()" maxlength="100" id="vcard_street" placeholder="{{ __('Street') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('City') }}</label>\
                                                <input type="text" class="form-control" name="vcard_city" onchange="regenerateQrCode()" id="vcard_city" maxlength="50" placeholder="{{ __('City') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Postal Code') }}</label>\
                                                <input type="text" class="form-control" name="vcard_postal" onchange="regenerateQrCode()" id="vcard_postal" maxlength="25" placeholder="{{ __('Postal Code') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Region') }}</label>\
                                                <input type="text" class="form-control" name="vcard_region" onchange="regenerateQrCode()" id="vcard_region" maxlength="50" placeholder="{{ __('Region') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Country') }}</label>\
                                                <input type="text" class="form-control" name="vcard_country" onchange="regenerateQrCode()" id="vcard_country" maxlength="50" placeholder="{{ __('Country') }}" required>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('More Info') }}</label>\
                                                <textarea name="vcard_note" id="vcard_note" class="form-control" cols="30" rows="2" maxlength="100" onchange="regenerateQrCode()" required></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Paypal
            if (type == "paypal") {
                // Paypal
                $('#selectedQrCodeType').val('paypal');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Paypal.Me Link') }}</label>\
                                                <div class="input-group input-group-flat">\
                                                    <span class="input-group-text">{{ __('https://paypal.me/') }}</span>\
                                                    <input type="text" class="form-control ps-0" name="paypal_link" id="paypal_link" onchange="regenerateQrCode()" placeholder="{{ __('yourpaypalmelink') }}" autocomplete="off" required>\
                                                </div>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }

            // Check Upi
            if (type == "upi") {
                // Upi
                $('#selectedQrCodeType').val('upi');
                generateCode = '<div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('UPI ID / VPA') }}</label>\
                                                <input type="text" class="form-control" onchange="regenerateQrCode()" maxlength="150" placeholder="{{ __('Eg: abi@okicici') }}" name="upi_id" id="upi_id" required autocomplete="off">\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Payee Name') }}</label>\
                                                <input type="text" class="form-control" onchange="regenerateQrCode()" maxlength="50" placeholder="{{ __('Eg: Abi') }}" name="upi_payee_name" id="upi_payee_name" required autocomplete="off">\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label required">{{ __('Currency') }}</label>\
                                                <select class="form-select" name="upi_currency" id="upi_currency" onchange="regenerateQrCode()" required>\
                                                    @include('user.pages.qr-codes.includes.currency')\
                                                </select>\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label">{{ __('Amount') }}</label>\
                                                <input type="number" min="0" step="0.001" max="100000" placeholder="{{ __('Eg: 500') }}" class="form-control" onchange="regenerateQrCode()" placeholder="{{ __('Amount') }}" name="upi_amount" id="upi_amount" autocomplete="off">\
                                            </div>\
                                        </div>\
                                        <div class="col-md-12 col-xl-12">\
                                            <div class="mb-3">\
                                                <label class="form-label">{{ __('Transaction Remark') }}</label>\
                                                <textarea name="upi_msg" id="upi_msg" maxlength="150" class="form-control" onchange="regenerateQrCode()" placeholder="{{ __('Transaction Remark') }}" cols="30" rows="2"></textarea>\
                                            </div>\
                                        </div>';
                $('#generate').html(generateCode);
            }
        }

        // Regenerate QR Code
        function regenerateQrCode() {
            "use strict";
            var qrcode_type = $("#selectedQrCodeType").val();
            var phone = $("#phone").val();
            var email = $("#email").val();
            var subject = $("#subject").val();
            var content = $("#content").val();
            var enable_analytics = $("#enable_analytics").prop("checked");
            var latitude = $("#latitude").val();
            var longitude = $("#longitude").val();
            var wifi_ssid = $("#wifi_ssid").val();
            var wifi_encryption = $("#wifi_encryption").val();
            var wifi_password = $("#wifi_password").val();
            var wifi_is_hidden = $("#wifi_is_hidden").val();
            var event = $("#event").val();
            var event_location = $("#event_location").val();
            var event_note = $("#event_note").val();
            var event_start_datetime = $("#event_start_datetime").val();
            var event_end_datetime = $("#event_end_datetime").val();
            var crypto_coin = $("#crypto_coin").val();
            var crypto_address = $("#crypto_address").val();
            var crypto_amount = $("#crypto_amount").val();
            var crypto_msg = $("#crypto_msg").val();

            var vcard_first_name = $("#vcard_first_name").val();
            var vcard_last_name = $("#vcard_last_name").val();
            var vcard_phone = $("#vcard_phone").val();
            var vcard_email = $("#vcard_email").val();
            var vcard_url = $("#vcard_url").val();
            var vcard_company = $("#vcard_company").val();
            var vcard_job_title = $("#vcard_job_title").val();
            var vcard_birthday = $("#vcard_birthday").val();
            var vcard_street = $("#vcard_street").val();
            var vcard_city = $("#vcard_city").val();
            var vcard_postal = $("#vcard_postal").val();
            var vcard_region = $("#vcard_region").val();
            var vcard_country = $("#vcard_country").val();
            var vcard_note = $("#vcard_note").val();
            var paypal_link = $("#paypal_link").val();
            var upi_id = $("#upi_id").val();
            var upi_payee_name = $("#upi_payee_name").val();
            var upi_currency = $("#upi_currency").val();
            var upi_amount = $("#upi_amount").val();
            var upi_msg = $("#upi_msg").val();
            var qrcode_style = $('input[name="qrcode_style"]:checked').val();
            var color_style = $('input[name="color_style"]:checked').val();
            var color = $("#color").val();
            var foreground_gradient_type = $('input[name="foreground_gradient_type"]:checked').val();
            var primary_color = $("#primary_color").val();
            var secondary_color = $("#secondary_color").val();
            var background_color = "#ffffff";
            var background_color_transparency = 0;
            var need_eye_color = $('input[name="need_eye_color"]:checked').val();
            var eye_color = $("#eye_color").val();
            var eyeColor_position = $('input[name="eyeColor_position"]:checked').val();
            var eye_color_style = $('input[name="eye_color_style"]:checked').val();
            var upload_logo = $("#upload_logo").prop('files')[0];
            var upload_logo_size = $('input[name="upload_logo_size"]:checked').val();
            var size = $('input[name="size"]:checked').val();
            var qrcode_ecc = $('input[name="qrcode_ecc"]:checked').val();

            // Form Data
            var formData = new FormData();

            formData.append('qrcode_type', qrcode_type);
            formData.append('email', email);
            formData.append('subject', subject);
            formData.append('content', content);
            formData.append('phone', phone);
            formData.append('enable_analytics', enable_analytics);
            formData.append('latitude', latitude);
            formData.append('longitude', longitude);
            formData.append('wifi_ssid', wifi_ssid);
            formData.append('wifi_encryption', wifi_encryption);
            formData.append('wifi_password', wifi_password);
            formData.append('wifi_is_hidden', wifi_is_hidden);
            formData.append('event', event);
            formData.append('event_location', event_location);
            formData.append('event_note', event_note);
            formData.append('event_start_datetime', event_start_datetime);
            formData.append('event_end_datetime', event_end_datetime);
            formData.append('crypto_coin', crypto_coin);
            formData.append('crypto_address', crypto_address);
            formData.append('crypto_amount', crypto_amount);
            formData.append('crypto_msg', crypto_msg);
            formData.append('vcard_first_name', vcard_first_name);
            formData.append('vcard_last_name', vcard_last_name);
            formData.append('vcard_phone', vcard_phone);
            formData.append('vcard_email', vcard_email);
            formData.append('vcard_url', vcard_url);
            formData.append('vcard_company', vcard_company);
            formData.append('vcard_job_title', vcard_job_title);
            formData.append('vcard_birthday', vcard_birthday);
            formData.append('vcard_street', vcard_street);
            formData.append('vcard_city', vcard_city);
            formData.append('vcard_postal', vcard_postal);
            formData.append('vcard_region', vcard_region);
            formData.append('vcard_country', vcard_country);
            formData.append('vcard_note', vcard_note);
            formData.append('paypal_link', paypal_link);
            formData.append('upi_id', upi_id);
            formData.append('upi_payee_name', upi_payee_name);
            formData.append('upi_currency', upi_currency);
            formData.append('upi_amount', upi_amount);
            formData.append('upi_msg', upi_msg);
            formData.append('qrcode_style', qrcode_style);
            formData.append('color_style', color_style);
            formData.append('color', color);
            formData.append('foreground_gradient_type', foreground_gradient_type);
            formData.append('primary_color', primary_color);
            formData.append('secondary_color', secondary_color);
            formData.append('background_color', background_color);
            formData.append('background_color_transparency', background_color_transparency);
            formData.append('need_eye_color', need_eye_color);
            formData.append('eye_color', eye_color);
            formData.append('eyeColor_position', eyeColor_position);
            formData.append('eye_color_style', eye_color_style);
            formData.append('upload_logo', upload_logo);
            formData.append('upload_logo_size', upload_logo_size);
            formData.append('size', size);
            formData.append('qrcode_ecc', qrcode_ecc);

            // Show loader
            $("#regenerate_qr").hide();
            $("#loader").show();

            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                /* the route pointing to the post function */
                url: `{{ route('user.regenerate.qr') }}`,
                method: 'POST',
                /* send the csrf-token and the input to the controller */
                data: formData,
                cache: false,
                processData: false,
                contentType: false,
                /* remind that 'data' is the response of the AjaxController */
                success: function(data) {
                    // Show qr code
                    $("#regenerate_qr").html("<img src='" + data.source + "'>");
                    $("#download_qr").attr("href", data.source);
                    $("#regenerate_qr").show();
                    $("#loader").hide();

                }
            });
        }

        // Download QR
        function downloadQR(type) {
            "use strict";
            var qrcodeSvgSrc = $("#regenerate_qr").children().attr("src");
            var qrcodeName = $("#name").val() == "" ? 'QrCode' : $("#name").val();
            var qrcodeSize = $('input[name="size"]:checked').val();

            // Call download function
            svgToQrCodeDownload(qrcodeSvgSrc, type, qrcodeName, qrcodeSize);
        }
    </script>
@endsection
@endsection
