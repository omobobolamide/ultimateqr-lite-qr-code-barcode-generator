@extends('layouts.guest')

{{-- Custom CSS --}}
@section('custom-css')
<title>{{ __('Free QR Generator') }}</title>
@endsection

@section('content')

    {{-- Topbar --}}
    @include('website.includes.topbar')

    @php
        use App\Models\Setting;
        $settings = Setting::where('status', 1)->first();
    @endphp

    {{-- URL --}}
    <section class="py-14 xl:pt-18 xl:pb-18 bg-white"
        style="background-image: url('{{ asset('images/web/elements/pattern-white.svg') }}'); background-position: center;">
        <div class="container px-4 mx-auto">
            <div class="flex flex-wrap" id="tabs-id">
                <div class="w-full">
                    {{-- Available QRs --}}
                    <div align="center" class="mt-4 mb-20">
                        <label for="chooseQr"
                            class="block mb-2 text-4xl font-medium text-gray-900 dark:text-gray-400">{{ __('Choose your Free QR Generator') }}</label>
                        <select id="chooseQr"
                            class="text-sm font-bold px-5 py-3 shadow-lg rounded block leading-normal text-gray-900 drop-shadow-2xl">
                            <option value="text" {{ Request::segment(2) == 'text' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->text == true ? '' : 'disabled' }}>
                                {{ __('Plain Text') }}
                            </option>
                            <option value="url" {{ Request::segment(2) == 'url' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->url == true ? '' : 'disabled' }}>
                                {{ __('URL') }}</option>
                            <option value="phone" {{ Request::segment(2) == 'phone' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->phone == true ? '' : 'disabled' }}>
                                {{ __('Phone Number') }}</option>
                            <option value="sms" {{ Request::segment(2) == 'sms' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->sms == true ? '' : 'disabled' }}>
                                {{ __('Text Message') }}
                            </option>
                            <option value="email" {{ Request::segment(2) == 'email' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->email == true ? '' : 'disabled' }}>
                                {{ __('Email') }}
                            </option>
                            <option value="whatsapp" {{ Request::segment(2) == 'whatsapp' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->whatsapp == true ? '' : 'disabled' }}>
                                {{ __('WhatsApp Message') }}</option>
                            <option value="facetime" {{ Request::segment(2) == 'facetime' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->facetime == true ? '' : 'disabled' }}>
                                {{ __('Facetime') }}</option>
                            <option value="location" {{ Request::segment(2) == 'location' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->location == true ? '' : 'disabled' }}>
                                {{ __('Map Location') }}</option>
                            <option value="wifi" {{ Request::segment(2) == 'wifi' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->wifi == true ? '' : 'disabled' }}>
                                {{ __('WiFi') }}
                            </option>
                            <option value="event" {{ Request::segment(2) == 'event' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->event == true ? '' : 'disabled' }}>
                                {{ __('Event') }}
                            </option>
                            <option value="vcard" {{ Request::segment(2) == 'contact' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->vcard == true ? '' : 'disabled' }}>
                                {{ __('Save Contact') }}</option>
                            <option value="crypto" {{ Request::segment(2) == 'crypto' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->crypto == true ? '' : 'disabled' }}>
                                {{ __('Crypto Payment') }}</option>
                            <option value="paypal" {{ Request::segment(2) == 'paypal' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->paypal == true ? '' : 'disabled' }}>
                                {{ __('Paypal Payment') }}</option>
                            <option value="upi" {{ Request::segment(2) == 'upi' ? 'selected' : '' }}
                                {{ json_decode($settings->accessable_qr)->upi == true ? '' : 'disabled' }}>
                                {{ __('UPI Payment') }}
                            </option>
                        </select>
                    </div>

                    <div class="relative flex flex-col min-w-0 break-words bg-white w-full my-6 shadow-2xl rounded">
                        <div class="px-4 py-5 flex-auto shadow-inner">
                            <div class="tab-content tab-space">
                                <div class="block" id="tab-text">
                                    <form action="{{ route('web.generate.qr') }}" method="post" enctype="multipart/form-data">
                                        @csrf

                                        <div class="grid lg:grid-cols-2 gap-1">
                                            <div>
                                                {{-- Regenerate QR Code --}}
                                                <div id="regenerate_qr" class="visible-print p-3 text-center">
                                                    <img src="data:image/png;base64, {!! base64_encode(
                                                        QrCode::format('png')->size('400')->merge('images/web/social/google_logo_new.png', 0.2, true)->generate('https://google.co.in'),
                                                    ) !!} ">
                                                </div>

                                                {{-- Loader --}}
                                                <div id="loader">
                                                    <img
                                                        src="data:image/gif;base64,">
                                                </div>
                                                <br>

                                                {{-- Download Formats --}}
                                                <h3 class="mb-4 text-2xl md:text-2xl font-bold">
                                                    {{ __('Download Formats') }}
                                                </h3>
                                                <div class="grid grid-cols-4 gap-1">
                                                    {{-- .PNG --}}
                                                    <div class="col-6 col-sm-4 col-md-2 col-xl cursor-pointer mb-3">
                                                        <a onclick="downloadQR('png')"
                                                            class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-{{ $config[11]->config_value }}-600 hover:bg-{{ $config[11]->config_value }}-700 rounded-l-xl rounded-t-xl">
                                                            {{ __('.PNG') }}
                                                        </a>
                                                    </div>

                                                    {{-- .JPG --}}
                                                    <div class="col-6 col-sm-4 col-md-2 col-xl cursor-pointer mb-3">
                                                        <a onclick="downloadQR('jpg')"
                                                            class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-{{ $config[11]->config_value }}-600 hover:bg-{{ $config[11]->config_value }}-700 rounded-l-xl rounded-t-xl">
                                                            {{ __('.JPG') }}
                                                        </a>
                                                    </div>

                                                    {{-- .SVG --}}
                                                    <div class="col-6 col-sm-4 col-md-2 col-xl cursor-pointer mb-3">
                                                        <a onclick="downloadQR('svg')"
                                                            class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-{{ $config[11]->config_value }}-600 hover:bg-{{ $config[11]->config_value }}-700 rounded-l-xl rounded-t-xl">
                                                            {{ __('.SVG') }}
                                                        </a>
                                                    </div>

                                                    {{-- .WebP --}}
                                                    <div class="col-6 col-sm-4 col-md-2 col-xl cursor-pointer mb-3">
                                                        <a onclick="downloadQR('webp')"
                                                            class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-{{ $config[11]->config_value }}-600 hover:bg-{{ $config[11]->config_value }}-700 rounded-l-xl rounded-t-xl">
                                                            {{ __('.WebP') }}
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>

                                            {{-- Column 2 --}}
                                            <div class="px-3">
                                                {{-- Selected Type --}}
                                                <input type="hidden" class="form-control" name="qrcode_type"
                                                    id="selectedQrCodeType" value="{{ Request::segment(2) }}">

                                                {{-- Default Generate Name --}}
                                                <input type="hidden" class="form-control" name="name"
                                                    value="{{ rand(1, 99999999999999) }}">

                                                {{-- Plan Content --}}
                                                @if (Request::segment(2) == 'text' && json_decode($settings->accessable_qr)->text == true)
                                                    <div class="mb-6">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Content') }}</label>
                                                        <textarea name="content" id="content" minlength="1" maxlength="350"
                                                            placeholder="Lorem Ipsum is simply dummy text of the printing and typesetting industry."
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" rows="2" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif


                                                {{-- URL --}}
                                                @if (Request::segment(2) == 'url' && json_decode($settings->accessable_qr)->url == true)
                                                    <div>
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium required">{{ __('Link Url') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="url"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="url" maxlength="80" onchange="regenerateQrCode()"
                                                            id="content" placeholder="{{ __(' Ex: http://domain.com') }}"
                                                            required>
                                                    </div>
                                                    <div class="inline-flex items-center mt-3 mb-3">
                                                        <label class="block mb-2 text-gray-800 font-medium">
                                                            <input class="form-checkbox h-5 w-5 text-red-600"
                                                                type="checkbox" name="enable_analytics"
                                                                id="enable_analytics" onclick="regenerateQrCode()">
                                                            <span
                                                                class="ml-2 text-gray-700">{{ __('Enable Analytics') }}</span>
                                                        </label>
                                                    </div>
                                                @endif

                                                {{-- Phone Number --}}
                                                @if (Request::segment(2) == 'phone' && json_decode($settings->accessable_qr)->phone == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Mobile Number') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            maxlength="25" name="content" onchange="regenerateQrCode()"
                                                            id="content" placeholder="{{ __('Mobile Number') }}"
                                                            required>
                                                    </div>
                                                @endif

                                                {{-- SMS --}}
                                                @if (Request::segment(2) == 'sms' && json_decode($settings->accessable_qr)->sms == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Mobile Number') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="phone" maxlength="25" onchange="regenerateQrCode()"
                                                            id="phone" placeholder="{{ __('Mobile Number') }}"
                                                            required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Message') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="content" id="content" maxlength="256"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" rows="5" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif

                                                {{-- SMS --}}
                                                @if (Request::segment(2) == 'email' && json_decode($settings->accessable_qr)->email == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Email ID') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="email" onchange="regenerateQrCode()" maxlength="50"
                                                            id="email" placeholder="{{ __('Email ID') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Subject') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="subject" onchange="regenerateQrCode()" maxlength="50"
                                                            id="subject" placeholder="{{ __('Subject') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Body') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="content" id="content"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" maxlength="300" rows="5" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif

                                                {{-- WhatsApp --}}
                                                @if (Request::segment(2) == 'whatsapp' && json_decode($settings->accessable_qr)->whatsapp == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('WhatsApp Number') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="phone" onchange="regenerateQrCode()" id="phone"
                                                            maxlength="25" placeholder="{{ __('WhatsApp Number') }}"
                                                            required>
                                                        <small
                                                            class="mt-2">{{ __('Include country code (For ex: 919876543210)') }}</small>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Body') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="content" id="content"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" maxlength="350" rows="5" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif

                                                {{-- Facetime --}}
                                                @if (Request::segment(2) == 'facetime' && json_decode($settings->accessable_qr)->facetime == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Mobile number/email ID') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            maxlength="80" name="content" onchange="regenerateQrCode()"
                                                            id="content"
                                                            placeholder="{{ __('Mobile number/email ID') }}" required>
                                                        <small
                                                            class="mt-2">{{ __('Only support on iOS & MacOS Devices') }}</small>
                                                    </div>
                                                @endif

                                                {{-- Location --}}
                                                @if (Request::segment(2) == 'location' && json_decode($settings->accessable_qr)->location == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Latitude') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            maxlength="50" name="latitude" onchange="regenerateQrCode()"
                                                            id="latitude" placeholder="{{ __('Latitude') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Longitude') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            maxlength="50" name="longitude" onchange="regenerateQrCode()"
                                                            id="longitude" placeholder="{{ __('Longitude') }}" required>
                                                    </div>
                                                @endif

                                                {{-- WiFi --}}
                                                @if (Request::segment(2) == 'wifi' && json_decode($settings->accessable_qr)->wifi == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Network Name (SSID)') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="wifi_ssid" maxlength="100"
                                                            onchange="regenerateQrCode()" id="wifi_ssid"
                                                            placeholder="{{ __('Network Name (SSID)') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="chooseQr"
                                                            class="block mb-2 font-medium text-gray-900 dark:text-gray-400">{{ __('Security') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <select id="wifi_encryption" name="wifi_encryption"
                                                            onchange="regenerateQrCode()"
                                                            class="w-full text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal text-gray-900 drop-shadow-2xl">
                                                            <option value="nopass">{{ __('None') }}</option>
                                                            <option value="WEP">{{ __('WEP') }}</option>
                                                            <option value="WPA">{{ __('WPA/WPA2-Personal') }}</option>
                                                        </select>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Password') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="wifi_password" maxlength="25"
                                                            onchange="regenerateQrCode()" id="wifi_password"
                                                            placeholder="{{ __('Password') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="chooseQr"
                                                            class="block mb-2 font-medium text-gray-900 dark:text-gray-400">{{ __('Hidden?') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <select id="wifi_is_hidden" name="wifi_is_hidden"
                                                            onchange="regenerateQrCode()"
                                                            class="w-full text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal text-gray-900 drop-shadow-2xl">
                                                            <option value="1">{{ __('Yes') }}</option>
                                                            <option value="0">{{ __('No') }}</option>
                                                        </select>
                                                    </div>
                                                @endif

                                                {{-- Event --}}
                                                @if (Request::segment(2) == 'event' && json_decode($settings->accessable_qr)->event == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Title') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="event" maxlength="80" onchange="regenerateQrCode()"
                                                            id="event" placeholder="{{ __('Title') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Message') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="event_note" id="event_note"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            placeholder="{{ __('Description') }}" maxlength="220" cols="30" rows="2"
                                                            onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Location') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="event_location" maxlength="80"
                                                            onchange="regenerateQrCode()" id="event_location"
                                                            placeholder="{{ __('Location') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('From') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="datetime-local"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="event_start_datetime" onchange="regenerateQrCode()"
                                                            id="event_start_datetime" placeholder="{{ __('From') }}"
                                                            required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('To') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="datetime-local"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="event_end_datetime" onchange="regenerateQrCode()"
                                                            id="event_end_datetime" placeholder="{{ __('To') }}"
                                                            required>
                                                    </div>
                                                @endif

                                                {{-- Crypto Payment --}}
                                                @if (Request::segment(2) == 'crypto' && json_decode($settings->accessable_qr)->crypto == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 font-medium text-gray-900 dark:text-gray-400">{{ __('Cryptocurrency') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <select
                                                            class="w-full text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal text-gray-900 drop-shadow-2xl"
                                                            name="crypto_coin" id="crypto_coin"
                                                            onchange="regenerateQrCode()" required>
                                                            <option value="bitcoin">{{ __('Bitcoin') }}</option>
                                                            <option value="bitcoincash">{{ __('Bitcoin cash address') }}
                                                            </option>
                                                            <option value="ethereum">{{ __('Ether') }}</option>
                                                            <option value="litecoin">{{ __('Litecoin') }}</option>
                                                            <option value="dash">{{ __('Dash') }}</option>
                                                        </select>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Receiver') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="crypto_address" maxlength="250"
                                                            onchange="regenerateQrCode()" id="crypto_address"
                                                            placeholder="{{ __('Receiver') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Amount') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="crypto_amount" maxlength="15"
                                                            onchange="regenerateQrCode()" id="crypto_amount"
                                                            placeholder="{{ __('Amount') }}" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Message') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="crypto_msg" id="crypto_msg"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" rows="2" maxlength="150" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif

                                                {{-- vCard --}}
                                                @if (Request::segment(2) == 'vcard' && json_decode($settings->accessable_qr)->vcard == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('First name') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_first_name" onchange="regenerateQrCode()"
                                                            maxlength="40" id="vcard_first_name"
                                                            placeholder="{{ __('First name') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Last name') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_last_name" onchange="regenerateQrCode()"
                                                            maxlength="40" id="vcard_last_name"
                                                            placeholder="{{ __('Last name') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Mobile number') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_phone" onchange="regenerateQrCode()"
                                                            id="vcard_phone" maxlength="20"
                                                            placeholder="{{ __('Mobile number') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Email Address') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="email"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_email" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_email"
                                                            placeholder="{{ __('Email Address') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Website Link') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="url"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_url" onchange="regenerateQrCode()" id="vcard_url"
                                                            maxlength="50" placeholder="{{ __('Website Link') }}"
                                                            required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Company Name') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_company" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_company"
                                                            placeholder="{{ __('Company Name') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Job Position') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_job_title" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_job_title"
                                                            placeholder="{{ __('Job Position') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('DOB') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="date"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_birthday" onchange="regenerateQrCode()"
                                                            id="vcard_birthday" placeholder="{{ __('DOB') }}"
                                                            required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Street') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_street" onchange="regenerateQrCode()"
                                                            maxlength="100" id="vcard_street"
                                                            placeholder="{{ __('Street') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('City') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_city" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_city"
                                                            placeholder="{{ __('City') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Postal Code') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_postal" onchange="regenerateQrCode()"
                                                            maxlength="25" id="vcard_postal"
                                                            placeholder="{{ __('Postal Code') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Region') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_region" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_region"
                                                            placeholder="{{ __('Region') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Country') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="vcard_country" onchange="regenerateQrCode()"
                                                            maxlength="50" id="vcard_country"
                                                            placeholder="{{ __('Country') }}" required>
                                                    </div>

                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('More Info') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <textarea name="vcard_note" id="vcard_note"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            cols="30" rows="2" maxlength="100" onchange="regenerateQrCode()" required></textarea>
                                                    </div>
                                                @endif

                                                {{-- Paypal payment --}}
                                                @if (Request::segment(2) == 'paypal' && json_decode($settings->accessable_qr)->paypal == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Paypal.Me Link') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            name="paypal_link" id="paypal_link"
                                                            onchange="regenerateQrCode()" maxlength="80"
                                                            placeholder="{{ __('yourpaypalmelink') }}"
                                                            autocomplete="off" required>
                                                    </div>
                                                @endif

                                                {{-- UPI payment --}}
                                                @if (Request::segment(2) == 'upi' && json_decode($settings->accessable_qr)->upi == true)
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('UPI ID / VPA') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            onchange="regenerateQrCode()" maxlength="150"
                                                            placeholder="{{ __('Eg: abi@okicici') }}" name="upi_id"
                                                            id="upi_id" required autocomplete="off">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Payee Name') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <input type="text"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            onchange="regenerateQrCode()" maxlength="50"
                                                            placeholder="{{ __('Eg: Abi') }}" name="upi_payee_name"
                                                            id="upi_payee_name" required autocomplete="off">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Currency') }}
                                                            <span class="text-red-600">*</span></label>
                                                        <select
                                                            class="w-full text-xs font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal text-gray-900 drop-shadow-2xl"
                                                            name="upi_currency" id="upi_currency"
                                                            onchange="regenerateQrCode()" required>
                                                            @include('admin.pages.qr-codes.includes.currency')
                                                        </select>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Amount') }}</label>
                                                        <input type="number" min="0" step="0.001"
                                                            max="100000" placeholder="{{ __('Eg: 500') }}"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            onchange="regenerateQrCode()"
                                                            placeholder="{{ __('Amount') }}" name="upi_amount"
                                                            id="upi_amount" autocomplete="off">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label
                                                            class="block mb-2 text-gray-800 font-medium">{{ __('Transaction Remark') }}</label>
                                                        <textarea name="upi_msg" id="upi_msg" maxlength="150"
                                                            class="appearance-none block w-full p-3 leading-5 text-gray-900 border border-gray-200 rounded-lg shadow-md placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
                                                            onchange="regenerateQrCode()" placeholder="{{ __('Transaction Remark') }}" cols="30" rows="2"></textarea>
                                                    </div>
                                                @endif

                                                {{-- Advance Settings --}}
                                                <div class="flex flex-row">
                                                    <div class="w-full">
                                                        <h3 class="mb-4 text-2xl md:text-2xl font-bold">
                                                            {{ __('Advanced Settings') }}</h3>
                                                        {{-- QR code settings --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('QR Code Style') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- QR Code Style --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_style"
                                                                            id="qrcode_style" value="square"
                                                                            onclick="regenerateQrCode()" checked />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-square"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect x="4" y="4" width="16"
                                                                                height="16" rx="2">
                                                                            </rect>
                                                                        </svg>
                                                                        {{ __('Square') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_style"
                                                                            id="qrcode_style" value="dot"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon mr-1 icon-tabler icon-tabler-grain"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <circle cx="4.5" cy="9.5" r="1">
                                                                            </circle>
                                                                            <circle cx="9.5" cy="4.5" r="1">
                                                                            </circle>
                                                                            <circle cx="9.5" cy="14.5" r="1">
                                                                            </circle>
                                                                            <circle cx="4.5" cy="19.5" r="1">
                                                                            </circle>
                                                                            <circle cx="14.5" cy="9.5" r="1">
                                                                            </circle>
                                                                            <circle cx="19.5" cy="4.5" r="1">
                                                                            </circle>
                                                                            <circle cx="14.5" cy="19.5" r="1">
                                                                            </circle>
                                                                            <circle cx="19.5" cy="14.5" r="1">
                                                                            </circle>
                                                                        </svg>
                                                                        {{ __('Dot') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">

                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_style"
                                                                            id="qrcode_style" value="round"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-border-radius"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path d="M4 12v-4a4 4 0 0 1 4 -4h4"></path>
                                                                            <line x1="16" y1="4"
                                                                                x2="16" y2="4.01"></line>
                                                                            <line x1="20" y1="4"
                                                                                x2="20" y2="4.01"></line>
                                                                            <line x1="20" y1="8"
                                                                                x2="20" y2="8.01"></line>
                                                                            <line x1="20" y1="12"
                                                                                x2="20" y2="12.01"></line>
                                                                            <line x1="4" y1="16"
                                                                                x2="4" y2="16.01"></line>
                                                                            <line x1="20" y1="16"
                                                                                x2="20" y2="16.01"></line>
                                                                            <line x1="4" y1="20"
                                                                                x2="4" y2="20.01"></line>
                                                                            <line x1="8" y1="20"
                                                                                x2="8" y2="20.01"></line>
                                                                            <line x1="12" y1="20"
                                                                                x2="12" y2="20.01"></line>
                                                                            <line x1="16" y1="20"
                                                                                x2="16" y2="20.01"></line>
                                                                            <line x1="20" y1="20"
                                                                                x2="20" y2="20.01"></line>
                                                                        </svg>
                                                                        {{ __('Round') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Color Style --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Color Style') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- QR Code Style --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="color_style"
                                                                            value="color"
                                                                            onclick="checkColor(this); regenerateQrCode();"
                                                                            checked />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-brush"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path d="M3 21v-4a4 4 0 1 1 4 4h-4"></path>
                                                                            <path d="M21 3a16 16 0 0 0 -12.8 10.2"></path>
                                                                            <path d="M21 3a16 16 0 0 1 -10.2 12.8"></path>
                                                                            <path d="M10.6 9a9 9 0 0 1 4.4 4.4"></path>
                                                                        </svg>

                                                                        {{ __('Single Color') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="color_style"
                                                                            value="gradient" id="gradient"
                                                                            onclick="checkColor(this); regenerateQrCode();" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-palette"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path
                                                                                d="M12 21a9 9 0 1 1 0 -18a9 8 0 0 1 9 8a4.5 4 0 0 1 -4.5 4h-2.5a2 2 0 0 0 -1 3.75a1.3 1.3 0 0 1 -1 2.25">
                                                                            </path>
                                                                            <circle cx="7.5" cy="10.5" r=".5"
                                                                                fill="currentColor"></circle>
                                                                            <circle cx="12" cy="7.5" r=".5"
                                                                                fill="currentColor">
                                                                            </circle>
                                                                            <circle cx="16.5" cy="10.5" r=".5"
                                                                                fill="currentColor"></circle>
                                                                        </svg>
                                                                        {{ __('Gradient Color') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Color --}}
                                                        <div>
                                                            <div class="mb-2 color-form-group">
                                                                <label
                                                                    class="text-1xl md:text-1xl font-bold">{{ __('Color') }}</label>
                                                                <input class="w-full p-1" type="color" name="color"
                                                                    id="color" value="#000000"
                                                                    onchange="regenerateQrCode()"
                                                                    placeholder="{{ __('Foreground color') }}" />
                                                            </div>
                                                        </div>

                                                        {{-- Gradient type --}}
                                                        <div class="mb-2 color-form-group gradient-form-group">
                                                            <label
                                                                class="text-1xl md:text-1xl font-bold">{{ __('Gradient type') }}</label>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Vertical --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2"
                                                                            name="foreground_gradient_type"
                                                                            id="foreground_gradient_type" value="vertical"
                                                                            onclick="regenerateQrCode()" checked />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-square"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect x="4" y="4" width="16"
                                                                                height="16" rx="2">
                                                                            </rect>
                                                                        </svg>
                                                                        {{ __('Vertical') }}</label>
                                                                </div>

                                                                {{-- Horizontal --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2"
                                                                            name="foreground_gradient_type"
                                                                            id="foreground_gradient_type"
                                                                            value="horizontal"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon mr-1 icon-tabler icon-tabler-grain"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <circle cx="4.5" cy="9.5" r="1">
                                                                            </circle>
                                                                            <circle cx="9.5" cy="4.5" r="1">
                                                                            </circle>
                                                                            <circle cx="9.5" cy="14.5" r="1">
                                                                            </circle>
                                                                            <circle cx="4.5" cy="19.5" r="1">
                                                                            </circle>
                                                                            <circle cx="14.5" cy="9.5" r="1">
                                                                            </circle>
                                                                            <circle cx="19.5" cy="4.5" r="1">
                                                                            </circle>
                                                                            <circle cx="14.5" cy="19.5" r="1">
                                                                            </circle>
                                                                            <circle cx="19.5" cy="14.5" r="1">
                                                                            </circle>
                                                                        </svg>
                                                                        {{ __('Horizontal') }}</label>
                                                                </div>

                                                                {{-- Diagonal --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2"
                                                                            name="foreground_gradient_type"
                                                                            id="foreground_gradient_type" value="diagonal"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-border-radius"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path d="M4 12v-4a4 4 0 0 1 4 -4h4"></path>
                                                                            <line x1="16" y1="4"
                                                                                x2="16" y2="4.01"></line>
                                                                            <line x1="20" y1="4"
                                                                                x2="20" y2="4.01"></line>
                                                                            <line x1="20" y1="8"
                                                                                x2="20" y2="8.01"></line>
                                                                            <line x1="20" y1="12"
                                                                                x2="20" y2="12.01"></line>
                                                                            <line x1="4" y1="16"
                                                                                x2="4" y2="16.01"></line>
                                                                            <line x1="20" y1="16"
                                                                                x2="20" y2="16.01"></line>
                                                                            <line x1="4" y1="20"
                                                                                x2="4" y2="20.01"></line>
                                                                            <line x1="8" y1="20"
                                                                                x2="8" y2="20.01"></line>
                                                                            <line x1="12" y1="20"
                                                                                x2="12" y2="20.01"></line>
                                                                            <line x1="16" y1="20"
                                                                                x2="16" y2="20.01"></line>
                                                                            <line x1="20" y1="20"
                                                                                x2="20" y2="20.01"></line>
                                                                        </svg>
                                                                        {{ __('Diagonal') }}</label>
                                                                </div>

                                                                {{-- Inverse diagonal --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2"
                                                                            name="foreground_gradient_type"
                                                                            id="foreground_gradient_type"
                                                                            value="inverse_diagonal"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-border-radius"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path d="M4 12v-4a4 4 0 0 1 4 -4h4"></path>
                                                                            <line x1="16" y1="4"
                                                                                x2="16" y2="4.01"></line>
                                                                            <line x1="20" y1="4"
                                                                                x2="20" y2="4.01"></line>
                                                                            <line x1="20" y1="8"
                                                                                x2="20" y2="8.01"></line>
                                                                            <line x1="20" y1="12"
                                                                                x2="20" y2="12.01"></line>
                                                                            <line x1="4" y1="16"
                                                                                x2="4" y2="16.01"></line>
                                                                            <line x1="20" y1="16"
                                                                                x2="20" y2="16.01"></line>
                                                                            <line x1="4" y1="20"
                                                                                x2="4" y2="20.01"></line>
                                                                            <line x1="8" y1="20"
                                                                                x2="8" y2="20.01"></line>
                                                                            <line x1="12" y1="20"
                                                                                x2="12" y2="20.01"></line>
                                                                            <line x1="16" y1="20"
                                                                                x2="16" y2="20.01"></line>
                                                                            <line x1="20" y1="20"
                                                                                x2="20" y2="20.01"></line>
                                                                        </svg>
                                                                        {{ __('Inverse diagonal') }}</label>
                                                                </div>

                                                                {{-- Radial --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2"
                                                                            name="foreground_gradient_type"
                                                                            id="foreground_gradient_type" value="radial"
                                                                            onclick="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-border-radius"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <path d="M4 12v-4a4 4 0 0 1 4 -4h4"></path>
                                                                            <line x1="16" y1="4"
                                                                                x2="16" y2="4.01"></line>
                                                                            <line x1="20" y1="4"
                                                                                x2="20" y2="4.01"></line>
                                                                            <line x1="20" y1="8"
                                                                                x2="20" y2="8.01"></line>
                                                                            <line x1="20" y1="12"
                                                                                x2="20" y2="12.01"></line>
                                                                            <line x1="4" y1="16"
                                                                                x2="4" y2="16.01"></line>
                                                                            <line x1="20" y1="16"
                                                                                x2="20" y2="16.01"></line>
                                                                            <line x1="4" y1="20"
                                                                                x2="4" y2="20.01"></line>
                                                                            <line x1="8" y1="20"
                                                                                x2="8" y2="20.01"></line>
                                                                            <line x1="12" y1="20"
                                                                                x2="12" y2="20.01"></line>
                                                                            <line x1="16" y1="20"
                                                                                x2="16" y2="20.01"></line>
                                                                            <line x1="20" y1="20"
                                                                                x2="20" y2="20.01"></line>
                                                                        </svg>
                                                                        {{ __('Radial') }}</label>
                                                                </div>
                                                            </div>

                                                            {{-- Primary color --}}
                                                            <div class="mb-2 color-form-group gradient-form-group">
                                                                <label
                                                                    class="block mb-2 text-gray-800 font-medium">{{ __('Primary color') }}</label>
                                                                <input class="w-full p-1" type="color"
                                                                    name="primary_color" id="primary_color"
                                                                    value="#0fcc8d" onchange="regenerateQrCode()"
                                                                    placeholder="{{ __('Primary color') }}" />
                                                            </div>

                                                            {{-- Secondary color --}}
                                                            <div class="mb-2 color-form-group gradient-form-group">
                                                                <label
                                                                    class="block mb-2 text-gray-800 font-medium">{{ __('Secondary color') }}</label>
                                                                <input class="w-full p-1" type="color"
                                                                    name="secondary_color" id="secondary_color"
                                                                    value="#088040" onchange="regenerateQrCode()"
                                                                    placeholder="{{ __('Secondary color') }}" />
                                                            </div>
                                                        </div>

                                                        {{-- Need eye color? --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Need eye color?') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Yes --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="need_eye_color"
                                                                            id="need_eye_color" value="1"
                                                                            onchange="checkEyeColor(this.value); regenerateQrCode();" />
                                                                        {{ __('Yes') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="need_eye_color"
                                                                            id="need_eye_color" value="0"
                                                                            onchange="checkEyeColor(this.value); regenerateQrCode();"
                                                                            checked />
                                                                        {{ __('No') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Change the eye colors --}}
                                                        <div class="mb-2 color-form-group eyes_control">
                                                            <label
                                                                class="block mb-2 text-gray-800 font-medium">{{ __('Change the eye colors') }}</label>
                                                            <input class="w-full p-1" type="color" name="eye_color"
                                                                id="eye_color" value="#000000"
                                                                onchange="regenerateQrCode()"
                                                                placeholder="{{ __('Change the eye colors') }}" />
                                                        </div>

                                                        {{-- EyeColor Position --}}
                                                        <div class="eyes_control">
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('EyeColor
                                                                                                                                                                                            Position') }}
                                                            </h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Left --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eyeColor_position"
                                                                            id="eyeColor_position" value="0"
                                                                            onchange="regenerateQrCode()" checked />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-float-left"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect width="6" height="6" x="4"
                                                                                y="5" rx="1">
                                                                            </rect>
                                                                            <line x1="14" y1="7"
                                                                                x2="20" y2="7"></line>
                                                                            <line x1="14" y1="11"
                                                                                x2="20" y2="11"></line>
                                                                            <line x1="4" y1="15"
                                                                                x2="20" y2="15"></line>
                                                                            <line x1="4" y1="19"
                                                                                x2="20" y2="19"></line>
                                                                        </svg>
                                                                        {{ __('Left') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eyeColor_position"
                                                                            id="eyeColor_position" value="1"
                                                                            onchange="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-float-right"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect width="6" height="6" x="14"
                                                                                y="5" rx="1">
                                                                            </rect>
                                                                            <line x1="4" y1="7"
                                                                                x2="10" y2="7"></line>
                                                                            <line x1="4" y1="11"
                                                                                x2="10" y2="11"></line>
                                                                            <line x1="4" y1="15"
                                                                                x2="20" y2="15"></line>
                                                                            <line x1="4" y1="19"
                                                                                x2="20" y2="19"></line>
                                                                        </svg>
                                                                        {{ __('Right') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eyeColor_position"
                                                                            id="eyeColor_position" value="2"
                                                                            onchange="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-float-center"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect width="6" height="6" x="9"
                                                                                y="5" rx="1">
                                                                            </rect>
                                                                            <line x1="4" y1="7"
                                                                                x2="5" y2="7"></line>
                                                                            <line x1="4" y1="11"
                                                                                x2="5" y2="11"></line>
                                                                            <line x1="19" y1="7"
                                                                                x2="20" y2="7"></line>
                                                                            <line x1="19" y1="11"
                                                                                x2="20" y2="11"></line>
                                                                            <line x1="4" y1="15"
                                                                                x2="20" y2="15"></line>
                                                                            <line x1="4" y1="19"
                                                                                x2="20" y2="19"></line>
                                                                        </svg>
                                                                        {{ __('Bottom') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Eye Color Style --}}
                                                        <div class="eyes_control">
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Eye Color Style') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Square --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eye_color_style"
                                                                            id="eye_color_style" value="square"
                                                                            onchange="regenerateQrCode()" checked />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-square"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect x="4" y="4" width="16"
                                                                                height="16" rx="2">
                                                                            </rect>
                                                                        </svg>
                                                                        {{ __('Square') }}</label>
                                                                </div>

                                                                {{-- Circle --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eye_color_style"
                                                                            id="eye_color_style" value="circle"
                                                                            onchange="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-circle"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <circle cx="12" cy="12" r="9">
                                                                            </circle>
                                                                        </svg>
                                                                        {{ __('Circle') }}</label>
                                                                </div>

                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="eyeColor_position"
                                                                            id="eyeColor_position" value="2"
                                                                            onchange="regenerateQrCode()" />
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                                            class="icon icon-tabler mr-1 icon-tabler-float-center"
                                                                            width="24" height="24"
                                                                            viewBox="0 0 24 24" stroke-width="2"
                                                                            stroke="currentColor" fill="none"
                                                                            stroke-linecap="round"
                                                                            stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z"
                                                                                fill="none">
                                                                            </path>
                                                                            <rect width="6" height="6" x="9"
                                                                                y="5" rx="1">
                                                                            </rect>
                                                                            <line x1="4" y1="7"
                                                                                x2="5" y2="7"></line>
                                                                            <line x1="4" y1="11"
                                                                                x2="5" y2="11"></line>
                                                                            <line x1="19" y1="7"
                                                                                x2="20" y2="7"></line>
                                                                            <line x1="19" y1="11"
                                                                                x2="20" y2="11"></line>
                                                                            <line x1="4" y1="15"
                                                                                x2="20" y2="15"></line>
                                                                            <line x1="4" y1="19"
                                                                                x2="20" y2="19"></line>
                                                                        </svg>
                                                                        {{ __('Bottom') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Merge Logo --}}
                                                        <div class="mb-2">
                                                            <label
                                                                class="block mb-2 text-gray-800 font-medium">{{ __('Upload Logo') }}</label>
                                                            <input class="w-full p-1" type="file"
                                                                class="form-control" name="upload_logo"
                                                                id="upload_logo" accept=".jpg, .jpeg, .png, .gif"
                                                                onchange="regenerateQrCode()" />
                                                        </div>

                                                        {{-- Upload Logo size --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Upload Logo Size') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Small --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="upload_logo_size"
                                                                            id="upload_logo_size" value="0.1"
                                                                            onclick="regenerateQrCode()" />
                                                                        {{ __('Small') }}</label>
                                                                </div>

                                                                {{-- Medium --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="upload_logo_size"
                                                                            id="upload_logo_size" value="0.2"
                                                                            onclick="regenerateQrCode()" checked />
                                                                        {{ __('Medium') }}</label>
                                                                </div>

                                                                {{-- Large --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="upload_logo_size"
                                                                            id="upload_logo_size" value="0.5"
                                                                            onclick="regenerateQrCode()" />
                                                                        {{ __('Large') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- More setting --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Download Quality') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Basic --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="size"
                                                                            id="size" value="200"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('Basic') }}</label>
                                                                </div>

                                                                {{-- Normal --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="size"
                                                                            id="size" value="500"
                                                                            onchange="regenerateQrCode()" checked />
                                                                        {{ __('Normal') }}</label>
                                                                </div>

                                                                {{-- Fine --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="size"
                                                                            id="size" value="1000"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('Fine') }}</label>
                                                                </div>

                                                                {{-- High --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="size"
                                                                            id="size" value="1500"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('High') }}</label>
                                                                </div>

                                                                {{-- Very High --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="size"
                                                                            id="size" value="2000"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('Very High') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        {{-- Level of error correction --}}
                                                        <div>
                                                            <h3 class="mb-4 text-1xl md:text-1xl font-bold">
                                                                {{ __('Level of error correction') }}</h3>
                                                            <div class="mx-auto text-center flex flex-wrap pt-1">
                                                                {{-- Low --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_ecc"
                                                                            id="qrcode_ecc" value="L"
                                                                            onchange="regenerateQrCode()" checked />
                                                                        {{ __('Low') }}</label>
                                                                </div>

                                                                {{-- Medium --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_ecc"
                                                                            id="qrcode_ecc" value="M"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('Medium') }}</label>
                                                                </div>

                                                                {{-- Quartile --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_ecc"
                                                                            id="qrcode_ecc" value="Q"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('Quartile') }}</label>
                                                                </div>

                                                                {{-- High --}}
                                                                <div class="flex items-center mr-4 mb-4">
                                                                    <label class="flex items-center cursor-pointer">
                                                                        <input type="radio" class="mr-2" name="qrcode_ecc"
                                                                            id="qrcode_ecc" value="H"
                                                                            onchange="regenerateQrCode()" />
                                                                        {{ __('High') }}</label>
                                                                </div>
                                                            </div>
                                                        </div>

                                                        <div class="text-left mt-3">
                                                            <div class="text-end">
                                                                {{-- Check Signup --}}
                                                                @guest
                                                                    <a href="{{ route('login') }}"
                                                                        class="block px-4 py-3 mb-2 leading-loose text-md text-center text-white font-semibold bg-{{ $config[11]->config_value }}-600 hover:bg-{{ $config[11]->config_value }}-700 rounded-l-xl rounded-t-xl">
                                                                        {{ __('Sign Up') }}
                                                                    </a>
                                                                @endguest

                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    {{-- Footer --}}
    @include('website.includes.footer')

    {{-- Custom JS --}}
@section('custom-js')
    <script type="text/javascript" src="{{ asset('js/qr.js') }}"></script>
    <script src="{{ asset('js/sweetalert2.js') }}"></script>
    <script>
        $('#chooseQr').change(function() {
            // set the window's location property to the value of the option the user has selected
            window.location = `{{ URL::to('/') }}` + '/g/' + $(this).val();
        });
    </script>
    <script>
        // Check current color type
        window.onload = checkColor(`single`);
        // Check current eye color
        window.onload = checkEyeColor(`0`);

        // Check gradient
        hideWebGradientElements();

        // Hide loader
        $("#loader").hide();
        $("#regenerate_qr").show();

        // Default Download Image
        $("#download_qr").attr("href",
            "data:image/png;base64,  "
        );

        // Regenerate QR Code
        function regenerateQrCode() {
            "use strict";
            var qrcode_type = $("#selectedQrCodeType").val();
            var phone = $("#phone").val();
            var email = $("#email").val();
            var subject = $("#subject").val();
            var content = $("#content").val();
            var enable_analytics = $("#enable_analytics").prop("checked");
            var latitude = $("#latitude").val();
            var longitude = $("#longitude").val();
            var wifi_ssid = $("#wifi_ssid").val();
            var wifi_encryption = $("#wifi_encryption").val();
            var wifi_password = $("#wifi_password").val();
            var wifi_is_hidden = $("#wifi_is_hidden").val();
            var event = $("#event").val();
            var event_location = $("#event_location").val();
            var event_note = $("#event_note").val();
            var event_start_datetime = $("#event_start_datetime").val();
            var event_end_datetime = $("#event_end_datetime").val();
            var crypto_coin = $("#crypto_coin").val();
            var crypto_address = $("#crypto_address").val();
            var crypto_amount = $("#crypto_amount").val();
            var crypto_msg = $("#crypto_msg").val();

            var vcard_first_name = $("#vcard_first_name").val();
            var vcard_last_name = $("#vcard_last_name").val();
            var vcard_phone = $("#vcard_phone").val();
            var vcard_email = $("#vcard_email").val();
            var vcard_url = $("#vcard_url").val();
            var vcard_company = $("#vcard_company").val();
            var vcard_job_title = $("#vcard_job_title").val();
            var vcard_birthday = $("#vcard_birthday").val();
            var vcard_street = $("#vcard_street").val();
            var vcard_city = $("#vcard_city").val();
            var vcard_postal = $("#vcard_postal").val();
            var vcard_region = $("#vcard_region").val();
            var vcard_country = $("#vcard_country").val();
            var vcard_note = $("#vcard_note").val();
            var paypal_link = $("#paypal_link").val();
            var upi_id = $("#upi_id").val();
            var upi_payee_name = $("#upi_payee_name").val();
            var upi_currency = $("#upi_currency").val();
            var upi_amount = $("#upi_amount").val();
            var upi_msg = $("#upi_msg").val();
            var qrcode_style = $('input[name="qrcode_style"]:checked').val();
            var color_style = $('input[name="color_style"]:checked').val();
            var color = $("#color").val();
            var foreground_gradient_type = $('input[name="foreground_gradient_type"]:checked').val();
            var primary_color = $("#primary_color").val();
            var secondary_color = $("#secondary_color").val();
            var background_color = "#ffffff";
            var background_color_transparency = 0;
            var need_eye_color = $('input[name="need_eye_color"]:checked').val();
            var eye_color = $("#eye_color").val();
            var eyeColor_position = $('input[name="eyeColor_position"]:checked').val();
            var eye_color_style = $('input[name="eye_color_style"]:checked').val();
            var upload_logo = $("#upload_logo").prop('files')[0];
            var upload_logo_size = $('input[name="upload_logo_size"]:checked').val();
            var size = $('input[name="size"]:checked').val();
            var qrcode_ecc = $('input[name="qrcode_ecc"]:checked').val();

            // Form Data
            var formData = new FormData();

            formData.append('qrcode_type', qrcode_type);
            formData.append('email', email);
            formData.append('subject', subject);
            formData.append('content', content);
            formData.append('phone', phone);
            formData.append('enable_analytics', enable_analytics);
            formData.append('latitude', latitude);
            formData.append('longitude', longitude);
            formData.append('wifi_ssid', wifi_ssid);
            formData.append('wifi_encryption', wifi_encryption);
            formData.append('wifi_password', wifi_password);
            formData.append('wifi_is_hidden', wifi_is_hidden);
            formData.append('event', event);
            formData.append('event_location', event_location);
            formData.append('event_note', event_note);
            formData.append('event_start_datetime', event_start_datetime);
            formData.append('event_end_datetime', event_end_datetime);
            formData.append('crypto_coin', crypto_coin);
            formData.append('crypto_address', crypto_address);
            formData.append('crypto_amount', crypto_amount);
            formData.append('crypto_msg', crypto_msg);
            formData.append('vcard_first_name', vcard_first_name);
            formData.append('vcard_last_name', vcard_last_name);
            formData.append('vcard_phone', vcard_phone);
            formData.append('vcard_email', vcard_email);
            formData.append('vcard_url', vcard_url);
            formData.append('vcard_company', vcard_company);
            formData.append('vcard_job_title', vcard_job_title);
            formData.append('vcard_birthday', vcard_birthday);
            formData.append('vcard_street', vcard_street);
            formData.append('vcard_city', vcard_city);
            formData.append('vcard_postal', vcard_postal);
            formData.append('vcard_region', vcard_region);
            formData.append('vcard_country', vcard_country);
            formData.append('vcard_note', vcard_note);
            formData.append('paypal_link', paypal_link);
            formData.append('upi_id', upi_id);
            formData.append('upi_payee_name', upi_payee_name);
            formData.append('upi_currency', upi_currency);
            formData.append('upi_amount', upi_amount);
            formData.append('upi_msg', upi_msg);
            formData.append('qrcode_style', qrcode_style);
            formData.append('color_style', color_style);
            formData.append('color', color);
            formData.append('foreground_gradient_type', foreground_gradient_type);
            formData.append('primary_color', primary_color);
            formData.append('secondary_color', secondary_color);
            formData.append('background_color', background_color);
            formData.append('background_color_transparency', background_color_transparency);
            formData.append('need_eye_color', need_eye_color);
            formData.append('eye_color', eye_color);
            formData.append('eyeColor_position', eyeColor_position);
            formData.append('eye_color_style', eye_color_style);
            formData.append('upload_logo', upload_logo);
            formData.append('upload_logo_size', upload_logo_size);
            formData.append('size', size);
            formData.append('qrcode_ecc', qrcode_ecc);

            // Show loader
            $("#regenerate_qr").hide();
            $("#loader").show();

            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                /* the route pointing to the post function */
                url: `{{ route('web.generate.qr') }}`,
                method: 'POST',
                /* send the csrf-token and the input to the controller */
                data: formData,
                cache: false,
                processData: false,
                contentType: false,
                /* remind that 'data' is the response of the AjaxController */
                success: function(data) {
                    if (data.status == true) {
                        // Show qr code
                        $("#regenerate_qr").html("<img src='" + data.source + "'>");
                        $("#download_qr").attr("href", data.source);
                        $("#regenerate_qr").show();
                        $("#loader").hide();
                    } else {
                        $("#regenerate_qr").show();
                        $("#download_qr").attr("href", data.source);
                        $("#loader").hide();

                        Swal.fire({
                            icon: 'error',
                            title: `{{ __('Your limit is over') }}`,
                            showConfirmButton: false,
                            timer: 3000
                        })
                    }
                }
            });
        }

        // Download QR
        function downloadQR(type) {
            "use strict";
            var svgSrc = $("#regenerate_qr").children().attr("src");
            var name = $("#name").val() == null ? 'QrCode' : $("#name").val();
            var size = $('input[name="size"]:checked').val();

            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: `{{ route('qr.download.count') }}`,
                type: 'POST',
                success: function(result) {
                    if (result.status == true) {
                        // Call download function
                        svgToQrCodeDownload(svgSrc, type, name, size);
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: `{{ __('Your download limit has been reached. Try tomorrow.') }}`,
                            showConfirmButton: false,
                            timer: 3000
                        })
                    }
                }
            });
        }
    </script>
@endsection
@endsection
